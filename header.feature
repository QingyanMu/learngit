...
Feature: Header
  ...
  Scenario: An Ingress with annotation `bfe.ingress.kubernetes.io/rewrite-header.actions`
    ...
    And set the "key1" field for the request header with the value "value1" 
    And set the "key2" field for the request header with the value "value2" 
    And set the "key3" field for the request header with the value "value3" 
    And add the "key2" field for the request header with the value "_addvalue" 
    And delete the "key3" field for the request header with the value "value3" 